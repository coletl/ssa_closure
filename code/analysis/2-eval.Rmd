...
```{r predict}
percap[ , names(mods_percap) := lapply(mods_percap, predict, newdata = percap, 
                                       type = "response")]
...
```{r}
set_cols(percap, FUN = cutoff, j = grep("logit", names(percap), value = TRUE))
set_cols(absolute, FUN = cutoff, j = grep("logit", names(absolute), value = TRUE))
```
...
# Export favorite
```{r}
saveRDS(models$RF.absolute, "data/mod_RF_absolute.rds")
```
