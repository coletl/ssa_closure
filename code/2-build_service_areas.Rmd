...
states_sfl <- split(states_sf %>% st_transform(3857), states_sf$STUSPS)[offices$state]
off_sfl    <- split(offices %>% st_transform(3857), offices$state)[offices$state]
```
...
state_vorl <- 
  purrr::map(unique(offices$state),
...
system.time(
  ...
  parallel::mclapply(dates, 
                     function(.x)
                       Map(tessalate, 
                           office_sf = off_sfl[states], area_sf = states_sfl[states],
                           date = .x) %>% bind_rows(),
                     mc.cores = 6)
  )
