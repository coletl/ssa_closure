...
fips_list <- split(fips_codes, fips_codes$state)
```
...
sf3_state <- function(state, geography, variables, year = 2000, ...) {
  ...
  # Save immediately after downloading data in case some states fail
  fn <- sprintf("data/census/%s/%s_%s.rds", geography, state, year)
...
# ACS data
4x speed up from running in parallel. Should take a little more than a minute. For some reason, I can't grab the shapefiles at the same time.
```{r tract_par, eval = FALSE}
# options(tigris_use_cache = TRUE)

map(2010:2015,
    ~ mclapply(fips_codes$state, 
         function(st) acs_state(state = st, geography = "tract", year = .x, 
